...
void ePIC_DVCS_TASK::setMomCuts(Float_t factor = 1.){
  ...
  //std::cout<<"Max e, p = "<<fPMax_e<<"x"<<fPMax_p<<std::endl;
}
...
Bool_t ePIC_DVCS_TASK::applyCuts_Electron(std::vector<P3MVector> scate){
...
Bool_t ePIC_DVCS_TASK::applyCuts_Photon(std::vector<P3MVector> scatg){
   Bool_t passCuts{kTRUE};
...
Bool_t ePIC_DVCS_TASK::applyCuts_Proton(std::vector<P3MVector> scatp, TString sProtonDet="all"){
  Bool_t passCuts{kTRUE};
  ...
  // RP angluar acceptance: < 5.5 mrad
...
Bool_t ePIC_DVCS_TASK::applyCuts_All(P3MVector beame, P3MVector beamp, vector<P3MVector> scate, vector<P3MVector> scatp, vector<P3MVector> scatg, TString sProtonDet="all"){
  Bool_t passCuts{kTRUE};
...
// USE BEAM VECTORS
void ePIC_DVCS_TASK::undoAfterburnAndCalc(P3MVector& p, P3MVector& k){
...
void ePIC_DVCS_TASK::undoAfterburn(P3MVector& a){
...
// t = (p' - p)^2
Double_t ePIC_DVCS_TASK::calcT(P3MVector p, P3MVector pprime){
...
// t = (k - k' - g)^2
Double_t ePIC_DVCS_TASK::calcTNoP(P3MVector k, P3MVector kprime, P3MVector g){
...
// Q2 = (k - k')^2
Double_t ePIC_DVCS_TASK::calcQ2(P3MVector k, P3MVector kprime){
...
// xB = Q2 / 2(q.p)
Double_t ePIC_DVCS_TASK::calcBjorkenX(P3MVector k, P3MVector kprime, P3MVector p){
...
// y = (p.q)/(p.k)
Double_t ePIC_DVCS_TASK::calcBjorkenY(P3MVector k, P3MVector kprime, P3MVector p){
  P3MVector q = k - kprime;
...
// cos(phi_h) = (q x e).(q x p) / |q x e||q x p|
Double_t ePIC_DVCS_TASK::calcTrentoPhi(P3MVector k, P3MVector kprime, P3MVector pprime){  
  MomVector k3 = k.Vect();
  MomVector kp3 = kprime.Vect();
  MomVector pp3 = pprime.Vect();
  MomVector q3 = k3-kp3;

  MomVector qk = q3.Cross(k3);
  qk /= qk.R();
  MomVector qp = q3.Cross(pp3);
  qp /= qp.R();

  //return Angle(qk,qp);
  return TMath::ACos(qk.Dot(qp));
}

// Alternate hadronic plane angle calculation
// Angle between planes defined by [k, k'] and [p, p']
Double_t ePIC_DVCS_TASK::calcPhiH(P3MVector k, P3MVector kprime, P3MVector p, P3MVector pprime){ 
  MomVector k3 = k.Vect();
  MomVector kp3 = kprime.Vect();
  MomVector p3 = p.Vect();
  MomVector pp3 = pprime.Vect();

  MomVector kNorm = kp3.Cross(k3);
  kNorm /= kNorm.R();
  MomVector pNorm = pp3.Cross(p3);
  pNorm /= pNorm.R();
  ...
  return TMath::Sign(1.,pp3.Dot(kNorm))*Angle(kNorm,pNorm);
} 

// Calculate angle between planes of qp and qg
Double_t ePIC_DVCS_TASK::calcPhiQPQG(P3MVector k, P3MVector p, P3MVector kprime, P3MVector gprime){
  MomVector p3 = p.Vect();
  MomVector k3 = k.Vect();
  MomVector kp3 = kprime.Vect();
...
Double_t ePIC_DVCS_TASK::calcConeAngle(P3MVector k, P3MVector p, P3MVector kprime, P3MVector pprime, P3MVector gprime){
...
Double_t ePIC_DVCS_TASK::calcPMiss_3Body(P3MVector a, P3MVector b, P3MVector c, P3MVector d, P3MVector f){
  Float_t fPMiss = (a+b-c-d-f).P();
  return fPMiss;
}

Double_t ePIC_DVCS_TASK::calcPtMiss_3Body(P3MVector a, P3MVector b, P3MVector c, P3MVector d, P3MVector f){
  Float_t fPtMiss = (a+b-c-d-f).Pt();
  return fPtMiss;
}

// Calculation of missing energy for process ab->cdf
Double_t ePIC_DVCS_TASK::calcEMiss_3Body(P3MVector a, P3MVector b, P3MVector c, P3MVector d, P3MVector f){
  Float_t fEMiss = (a+b-c-d-f).E();
  return fEMiss;
}

// Calculation of missing mass (squared) for process ab->cdf
Double_t ePIC_DVCS_TASK::calcM2Miss_3Body(P3MVector a, P3MVector b, P3MVector c, P3MVector d, P3MVector f){
  Float_t fEMiss = (a+b-c-d-f).E();
  Float_t fPMiss = (a+b-c-d-f).P();
...
Double_t ePIC_DVCS_TASK::calcPMiss_2Body(P3MVector a, P3MVector b, P3MVector c, P3MVector d){
  Float_t fPMiss = (a+b-c-d).P();
  return fPMiss;
}

// Calculation of missing energy for process ab->cd
Double_t ePIC_DVCS_TASK::calcEMiss_2Body(P3MVector a, P3MVector b, P3MVector c, P3MVector d){
  Float_t fEMiss = (a+b-c-d).E();
  return fEMiss;
}

// Calculation of missing mass (squared) for process ab->cd
Double_t ePIC_DVCS_TASK::calcM2Miss_2Body(P3MVector a, P3MVector b, P3MVector c, P3MVector d){
  Float_t fEMiss = (a+b-c-d).E();
  Float_t fPMiss = (a+b-c-d).P();
...
void ePIC_DVCS_TASK::doAnalysis(){
  ...
  if(sSett != "hiAcc" && sSett != "hiDiv") sInList="./filelists/inputFileList_test_10x100Jun24.list";
  ...
  TH1D* h_eta_MCp   = new TH1D("eta_MCp",";#eta_{p'}(MC)", 250, -10.0, 10.0);
  ...
  TH1D* h_eta_MCe   = new TH1D("eta_MCe",";#eta_{e'}(MC)", 250, -10.0, 10.0);
  ...
  TH1D* h_eta_MCg   = new TH1D("eta_MCg",";#eta_{#gamma}(MC)", 250, -10.0, 10.0);
  ...
  TH1D* h_eta_MCAp   = new TH1D("eta_MCAp",";#eta_{p'}(MC|Reco)", 250, -10.0, 10.0);
  ...
  TH1D* h_eta_MCAe   = new TH1D("eta_MCAe",";#eta_{e'}(MC|Reco)", 250, -10.0, 10.0);
  ...
  TH1D* h_eta_MCAg   = new TH1D("eta_MCAg",";#eta_{#gamma}(MC|Reco)", 250, -10.0, 10.0);
  ...
  TH1D* h_eta_RPp   = new TH1D("eta_RPp",";#eta_{p'}(Reco)", 250, -10.0, 10.0);
  ...
  TH1D* h_eta_RPPp   = new TH1D("eta_RPPp",";#eta_{p'}(Reco)", 250, -10.0, 10.0);
  ...
  TH1D* h_eta_RPe   = new TH1D("eta_RPe",";#eta_{e'}(Reco)", 250, -10.0, 10.0);
  ...
  TH1D* h_eta_RPg   = new TH1D("eta_RPg",";#eta_{#gamma}(Reco)", 250, -10.0, 10.0);
  ...
  TH1D* h_t_MC    = new TH1D("t_MC"   , ";|t|(MC) [(GeV/c)^{2}]"  , 100, 0.0, 2.0);
  TH1D* h_Q2_MC   = new TH1D("Q2_MC"  , ";Q^{2}(MC) [(GeV/c)^{2}]", 500, 0.0, 10.0);
  TH1D* h_xB_MC   = new TH1D("xB_MC"  , ";log_{10}(x_{B})(MC)"    , 100, -5, 0);
  TH1D* h_y_MC    = new TH1D("y_MC"   , ";y(MC)"                  , 100, 0, 1);
  TH1D* h_TPhi_MC = new TH1D("tphi_MC", ";#phi_{h}(MC) [rad]"     , 350, -3.5, 3.5);

  // DVCS event kinematics - Associated MC particles
  TH1D* h_t_MCA    = new TH1D("t_MCA"   , ";|t|(MC|Reco) [(GeV/c)^{2}]"  , 100, 0.0, 2.0);
  TH1D* h_Q2_MCA   = new TH1D("Q2_MCA"  , ";Q^{2}(MC|Reco) [(GeV/c)^{2}]", 500, 0.0, 10.0);
  TH1D* h_xB_MCA   = new TH1D("xB_MCA"  , ";log_{10}(x_{B})(MC|Reco)"    , 100, -5, 0);
  TH1D* h_y_MCA    = new TH1D("y_MCA"   , ";y(MCA)"                      , 100, 0, 1);
  TH1D* h_TPhi_MCA = new TH1D("tphi_MCA", ";#phi_{h}(MC|Reco) [rad]"     , 350, -3.5, 3.5);
 
  // DVCS event kinematics - Reconstructed particles
  TH1D* h_t_RP     = new TH1D("t_RP"    , ";|t|(Reco) [(GeV/c)^{2}]"  , 100, 0.0, 2.0);
  TH1D* h_t_RPP    = new TH1D("t_RPP"   , ";|t|(Reco) [(GeV/c)^{2}]"  , 100, 0.0, 2.0); // Roman Pots
  TH1D* h_Q2_RP    = new TH1D("Q2_RP"   , ";Q^{2}(Reco) [(GeV/c)^{2}]", 500, 0.0, 10.0);
  TH1D* h_xB_RP    = new TH1D("xB_RP"   , ";log_{10}(x_{B})(Reco)"    , 100, -5, 0);
  TH1D* h_y_RP     = new TH1D("y_RP"    , ";y(Reco)"                  , 100, 0, 1);
  TH1D* h_TPhi_RP  = new TH1D("tphi_RP" , ";#phi_{h}(Reco) [rad]"     , 350, -3.5, 3.5);
  TH1D* h_TPhi_RPP = new TH1D("tphi_RPP", ";#phi_{h}(Reco) [rad]"     , 350, -3.5, 3.5); // Roman Pots

  // 2D plots, associated MC vs reconstructed
  TH2D* h_p_2d = new TH2D("h_p_2d",";p_{p'}(MC|Reco) [GeV/#it{c}];p_{p'}(Reco) [GeV/#it{c}]", 150, 0., 150., 150, 0., 150.);
  ...
  TH1D* h_M2miss3_MC = new TH1D("M2miss3_MC",";M^{2}_{miss}(MC) [(GeV/#it{c})^{2}]",200,-50,50);
  ...
  TH1D* h_M2miss3_MCA = new TH1D("M2miss3_MCA",";M^{2}_{miss}(MC|Reco) [(GeV/#it{c})^{2}]",200,-50,50);
  ...
  TH1D* h_M2miss3_RP = new TH1D("M2miss3_RP",";M^{2}_{miss}(Reco) [(GeV/#it{c})^{2}]",200,-50,50);
  // Kinematic conservation - partial final state
  TH1D* h_M2miss2ep_MC = new TH1D("M2miss2ep_MC",";M^{2}_{miss, e'p'}(MC) [(GeV/#it{c})^{2}]",200,-50,50);
  TH1D* h_M2miss2ep_MCA = new TH1D("M2miss2ep_MCA",";M^{2}_{miss, e'p'}(MCA) [(GeV/#it{c})^{2}]",200,-50,50);
  TH1D* h_M2miss2ep_RP = new TH1D("M2miss2ep_RP",";M^{2}_{miss, e'p'}(Reco) [(GeV/#it{c})^{2}]",200,-50,50);
  TH1D* h_M2miss2eg_MC = new TH1D("M2miss2eg_MC",";M^{2}_{miss, e'#gamma}(MC) [(GeV/#it{c})^{2}]",300,-500,1000);
  TH1D* h_M2miss2eg_MCA = new TH1D("M2miss2eg_MCA",";M^{2}_{miss, e'#gamma}(MCA) [(GeV/#it{c})^{2}]",300,-500,1000);
  TH1D* h_M2miss2eg_RP = new TH1D("M2miss2eg_RP",";M^{2}_{miss, e'#gamma}(Reco) [(GeV/#it{c})^{2}]",300,-500,1000);
  ...
  while(getline(fileListStream,fileName)){
    ...
    // Reconstructed/MC particle associations
    ...
    while (tree_reader.Next()){
      // 4-vectors for MC raw particles (INCLUDING BEAM)
      ...
      vector<P3MVector> scatp4_rec;   // Scattered proton (reconstructed)
      vector<P3MVector> scatp4_rom;   // Scattered proton (Roman Pots)
      ...
      //---------------------------------------------------------
      // 0. MC generated - FIND BEAM FIRST
	...
	if(mc_genStatus_array[imc] == 4){
	  ...
	  if(mc_pdg_array[imc] == 2212){ 
	    beamp4.SetCoordinates(mctrk.X(), mctrk.Y(), mctrk.Z(), mc_mass_array[imc]);
	  }
	  // Electron
	  else if(mc_pdg_array[imc] == 11){ 
	    beame4.SetCoordinates(mctrk.X(), mctrk.Y(), mctrk.Z(), mc_mass_array[imc]);
	  }		
	...
	if(mc_genStatus_array[mc_assoc_index] == 1){
	  if(mc_pdg_array[mc_assoc_index] == 2212){
	    scatp4_aso.push_back(q_assoc);
	  }
	  if(mc_pdg_array[mc_assoc_index] == 11){
	    scate4_aso.push_back(q_assoc);
	  }
	  if(mc_pdg_array[mc_assoc_index] == 22){
	...
	P3MVector q_reco(recotrk.X(),recotrk.Y(),recotrk.Z(),re_mass_array[ireco]);
	
	// Undo afterburner
	undoAfterburn(q_reco);
	
	// Need to identify protons NOT from the real reported PDG value.
	// Using combination of particle charge and mass instead
	if(TMath::Abs(re_mass_array[ireco]-0.938)<0.1 && re_charge_array[ireco]==1){
	  scatp4_rec.push_back(q_reco);
	}
	if(re_pdg_array[ireco] == 11){
	  scate4_rec.push_back(q_reco);
	}
	if(re_pdg_array[ireco] == 22){
	...
	h_Q2_MC->Fill(fQ2);
	h_y_MC->Fill(calcBjorkenY(beame4, scate4_gen[0], beamp4));
	fxB = calcBjorkenX(beame4, scate4_gen[0], beamp4);
	...
	if(applyCuts_Proton(scatp4_gen, "all")){
	  ...
	  ft = calcT(beamp4, scatp4_gen[0]);
	  if(applyCuts_DVCS("all")){
	    h_t_MC->Fill(calcT(beamp4, scatp4_gen[0]));
	    ...
	    //h_TPhi_MC->Fill(calcTrentoPhi(boost(beame4, vCoMi), boost(scate4_gen[0], vCoMf), boost(scatp4_gen[0], vCoMf)));
	    //h_TPhi_MC->Fill(calcTrentoPhi(beame4, scate4_gen[0], scatp4_gen[0]));
	    h_TPhi_MC->Fill(calcPhiH(boost(beame4, vCoMi), boost(scate4_gen[0], vCoMf), boost(beamp4, vCoMi), boost(scatp4_gen[0], vCoMf)));
	    //h_TPhi_MC->Fill(calcPhiH(beame4, scate4_gen[0], beamp4, scatp4_gen[0]));
      ...
      else fQ2 = calcQ2(beame4, scate4_aso[0]);
      // Fill histograms
      // Electron kinematics
	...
	// Electron-based event kinematics
	h_Q2_MCA->Fill(fQ2);
	h_y_MCA->Fill(calcBjorkenY(beame4, scate4_aso[0], beamp4));
	fxB = calcBjorkenX(beame4, scate4_aso[0], beamp4);
	...
	if(applyCuts_Proton(scatp4_aso, "all")){
	  ...
	  ft = calcT(beamp4, scatp4_aso[0]);
	  if(applyCuts_DVCS("all")){
	    h_t_MCA->Fill(calcT(beamp4, scatp4_aso[0]));
	    MomVector vCoMi = (beame4+beamp4).BoostToCM();
	    MomVector vCoMf = (scate4_aso[0]+scatp4_aso[0]+scatg4_aso[0]).BoostToCM();
	    //h_TPhi_MCA->Fill(calcTrentoPhi(boost(beame4, vCoMi), boost(scate4_aso[0], vCoMf), boost(scatp4_aso[0], vCoMf)));
	    //h_TPhi_MCA->Fill(calcTrentoPhi(beame4, scate4_aso[0], scatp4_aso[0]));
	    h_TPhi_MCA->Fill(calcPhiH(boost(beame4, vCoMi), boost(scate4_aso[0], vCoMf), boost(beamp4, vCoMi), boost(scatp4_aso[0], vCoMf)));
	    //h_TPhi_MCA->Fill(calcPhiH(beame4, scate4_aso[0], beamp4, scatp4_aso[0]));
	...
	// Electron-based event kinematics
	h_Q2_RP->Fill(fQ2);
	h_y_RP->Fill(calcBjorkenY(beame4, scate4_rec[0], beamp4));
	fxB = calcBjorkenX(beame4, scate4_rec[0], beamp4);
	...
	if(applyCuts_Proton(scatp4_rec, "B0")){
	  ...
	  ft = calcT(beamp4, scatp4_rec[0]);
	  if(applyCuts_DVCS("B0")){
	    h_t_RP->Fill(calcT(beamp4, scatp4_rec[0]));
	    MomVector vCoMi = (beame4+beamp4).BoostToCM();
	    MomVector vCoMf = (scate4_rec[0]+scatp4_rec[0]+scatg4_rec[0]).BoostToCM();
	    //h_TPhi_RP->Fill(calcTrentoPhi(boost(beame4, vCoMi), boost(scate4_rec[0], vCoMf), boost(scatp4_rec[0], vCoMf)));
	    //h_TPhi_RP->Fill(calcTrentoPhi(beame4, scate4_rec[0], scatp4_rec[0]));
	    h_TPhi_RP->Fill(calcPhiH(boost(beame4, vCoMi), boost(scate4_rec[0], vCoMf), boost(beamp4, vCoMi), boost(scatp4_rec[0], vCoMf)));
	    //h_TPhi_RP->Fill(calcPhiH(beame4, scate4_rec[0], beamp4, scatp4_rec[0]));
	    h_Cone_RP->Fill(calcConeAngle(beame4, beamp4, scate4_rec[0], scatp4_rec[0], scatg4_rec[0]));
	  }
	}
	
	// And proton information - RP
	if(applyCuts_Proton(scatp4_rom, "RP")){
	  // And event DVCS cuts
	  // Need to calculate missing mass squared and t for DVCS cuts
	  fM2miss = calcM2Miss_3Body(beame4, beamp4, scate4_rec[0], scatp4_rom[0], scatg4_rec[0]);
	  ft = calcT(beamp4, scatp4_rom[0]);
	  if(applyCuts_DVCS("RP")){
	    h_t_RPP->Fill(calcT(beamp4, scatp4_rom[0]));
	    MomVector vCoMi = (beame4+beamp4).BoostToCM();
	    MomVector vCoMf = (scate4_rec[0]+scatp4_rom[0]+scatg4_rec[0]).BoostToCM();
	    //h_TPhi_RPP->Fill(calcTrentoPhi(boost(beame4, vCoMi), boost(scate4_rec[0], vCoMf), boost(scatp4_rom[0], vCoMf)));
	    //h_TPhi_RPP->Fill(calcTrentoPhi(beame4, scate4_rec[0], scatp4_rom[0]));
	    h_TPhi_RPP->Fill(calcPhiH(boost(beame4, vCoMi), boost(scate4_rec[0], vCoMf), boost(beame4, vCoMi), boost(scatp4_rom[0], vCoMf)));
	    //h_TPhi_RPP->Fill(calcPhiH(beame4, scate4_rec[0], beamp4, scatp4_rom[0]));
	...
	h_t_2d->Fill(calcT(beamp4, scatp4_aso[0]), calcT(beamp4, scatp4_rec[0]));
      }
